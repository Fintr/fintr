...
```bash
ls -d .kilo 2>/dev/null && echo "kilo" \
  || (ls -d .ai/get-shit-done 2>/dev/null && echo "ai-repo") \
  || (ls -d .claude/get-shit-done 2>/dev/null && echo "claude") \
  || echo "unknown"
```

Use the detected root to resolve all canonical paths below. When layout is `ai-repo`, use `.ai/get-shit-done/` for workflows, references, templates, contexts, bin, and `agents/` under `.ai/get-shit-done/agents/`.
