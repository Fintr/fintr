...
class AiChatJob < ApplicationJob
  ...
  def perform(session_id, query, space_id, user_id)
    ...
    begin
      ...
      result = ::Ai::Operations::Rag::ProcessStreamingRagQuery.new.call(
        query: query,
        space_id: space_id
      )

      if result.failure?
        interaction&.update_with_error(result.failure.to_s)
        update_chat_cache(session_id, {
          status: "error",
          error: result.failure.to_s
        })
        return
      end
