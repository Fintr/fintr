...
describe("calculateHeaderSpacerHeight", () => {
  ...
  it("uses env() for iOS native regardless of JS safe-area arg (env() is source of truth)", () => {
    ...
    expect(heightWithLargeInset).toBe("calc(49px + env(safe-area-inset-top, 0px))")
    expect(heightWithSmallInset).toBe("calc(49px + env(safe-area-inset-top, 0px))")
    expect(heightWithZeroInset).toBe("calc(49px + env(safe-area-inset-top, 0px))")
  })
  ...
  it("applies a minimum top inset on Android native when inset is missing", () => {
    ...
    expect(androidHeight).toBe("calc(49px + 24px)")
  })

  it("uses the larger of native top inset and the Android minimum", () => {
    expect(calculateHeaderSpacerHeight(true, false, 5)).toBe("calc(49px + 24px)")
    expect(calculateHeaderSpacerHeight(true, false, 48)).toBe("calc(49px + 24px)")
  })
