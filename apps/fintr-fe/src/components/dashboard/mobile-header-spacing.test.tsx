...
vi.mock("@/hooks/async/useProAccess", () => ({
  useProAccess: () => ({
    data: { pro: false, source: "none", trialDaysRemaining: 0 },
    isPending: false,
  }),
}));
...
describe("Mobile Header Spacer - Top Safe Area", () => {
  describe("calculateHeaderSpacerHeight function", () => {
    it("should calculate Android native spacer with header body height + safe area", () => {
  ...
  describe("Dashboard Layout - Header Spacer", () => {
    it("should include spacer height that accounts for safe area", () => {
      ...
      // Android and Browser should include the sticky header body height
      expect(androidHeight).toContain("49px");
      expect(browserHeight).toContain("49px");

      // Android native is fixed to the header body height to avoid post-rotation drift
