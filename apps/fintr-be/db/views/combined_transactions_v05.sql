...
SELECT
  ...
  COALESCE(NULLIF(BTRIM(loans.description), ''), 'Loan — ' || entities.full_name) AS description,
  CASE
...
SELECT
  ...
  COALESCE(
    NULLIF(BTRIM(loan_payments.notes), ''),
    'Loan payment — ' || COALESCE(NULLIF(BTRIM(loans.description), ''), entities.full_name)
  ) AS description,
  CASE
    WHEN loans.loan_type = 'lent' THEN accounts.name
    ELSE NULL
