    ...
    module Receipts
      class FormatResult < Dry::Operation
        ...
        def format_extracted_data(params:)
          ...
          # Add nil checks
          return Failure(error: "Receipt data is missing") if receipt_data.nil?
          return Failure(error: "Confidence analysis is missing") if confidence_analysis.nil?
          return Failure(error: "Extracted fields are missing") if receipt_data[:extracted_fields].nil?
          return Failure(error: "Field confidence data is missing") if confidence_analysis[:field_confidence].nil?
