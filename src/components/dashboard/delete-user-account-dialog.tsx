'use client';

import { useState } from 'react';
import { useRouter } from 'next/navigation';
import { Button } from '../ui/button';
import { Input } from '../ui/input';
import { Label } from '../ui/label';
import {
  Dialog,
  DialogContent,
  DialogDescription,
  DialogFooter,
  DialogHeader,
  DialogTitle,
} from '../ui/dialog';
import { deleteAccount } from '@/services/auth/user/mutations';
import { useAuthApi } from '@/hooks/useAuthApi';
import { useAuth } from '@/contexts/AuthContext';
import { toast } from 'sonner';
import { AlertTriangle } from 'lucide-react';

interface DeleteUserAccountDialogProps {
  isOpen: boolean;
  onClose: () => void;
}

/**
 * Dialog component for permanently deleting the user's account.
 * Requires user to type "delete my account" to confirm the action.
 */
const DeleteUserAccountDialog = ({ isOpen, onClose }: DeleteUserAccountDialogProps) => {
  const router = useRouter();
  const { api } = useAuthApi();
  const { logout } = useAuth();
  const [confirmText, setConfirmText] = useState('');
  const [isLoading, setIsLoading] = useState(false);
  
  const CONFIRMATION_TEXT = 'delete my account';
  const isConfirmValid = confirmText.toLowerCase() === CONFIRMATION_TEXT;

  /**
   * Handles the account deletion process.
   * Makes API call, logs out the user, and redirects to login page.
   */
  const handleDeleteAccount = async () => {
    if (!isConfirmValid) return;
    
    setIsLoading(true);
    try {
      await deleteAccount({ api });
      toast.success('Account deleted successfully. Redirecting...');
      onClose();
      await logout();
      router.push('/login');
    } catch (error) {
      toast.error(`Failed to delete account: ${(error as Error).message}`);
      console.error('Failed to delete account:', error);
    } finally {
      setIsLoading(false);
    }
  };

  /**
   * Handles dialog close and resets form state.
   */
  const handleClose = () => {
    setConfirmText('');
    onClose();
  };

  return (
    <Dialog open={isOpen} onOpenChange={handleClose}>
      <DialogContent className="sm:max-w-md">
        <DialogHeader>
          <div className="flex items-center gap-3">
            <div className="flex h-10 w-10 items-center justify-center rounded-full bg-red-900/20">
              <AlertTriangle className="h-5 w-5 text-red-900" />
            </div>
            <div>
              <DialogTitle className="text-red-900">Delete Account</DialogTitle>
              <DialogDescription className="text-red-900">
                This action is permanent and cannot be undone.
              </DialogDescription>
            </div>
          </div>
        </DialogHeader>
        
        <div className="space-y-4">
          <div className="rounded-lg bg-red-100/50 p-4">
            <p className="text-sm text-red-800">
              <strong>Warning:</strong> This will permanently delete your entire account, 
              including all your financial data, transactions, accounts, budgets, goals, 
              and any spaces you own. You will be logged out and will not be able to recover 
              your account.
            </p>
          </div>
          
          <div className="space-y-2">
            <Label htmlFor="confirm-delete-text" className="text-sm font-medium">
              Type <span className="font-mono font-bold">"{CONFIRMATION_TEXT}"</span> to confirm:
            </Label>
            <Input
              id="confirm-delete-text"
              type="text"
              value={confirmText}
              onChange={(e) => setConfirmText(e.target.value)}
              placeholder="Type confirmation text here"
              className="font-mono"
              aria-label="Confirmation text input"
            />
          </div>
        </div>
        
        <DialogFooter className="gap-2">
          <Button
            variant="outline"
            onClick={handleClose}
            disabled={isLoading}
            aria-label="Cancel deletion"
          >
            Cancel
          </Button>
          <Button
            variant="destructive"
            onClick={handleDeleteAccount}
            disabled={!isConfirmValid || isLoading}
            aria-label="Confirm delete account"
          >
            {isLoading ? 'Deleting...' : 'Delete Account'}
          </Button>
        </DialogFooter>
      </DialogContent>
    </Dialog>
  );
};

export default DeleteUserAccountDialog;
