/**
 * In-App Google Sign-In for iOS using Capacitor Browser
...
export interface InAppGoogleSignInOptions {
  redirectUri?: string;
  state?: string;
}
 ...
 */
export const initiateInAppGoogleSignIn = async (options?: InAppGoogleSignInOptions) => {
  ...
  // Debug logging
  console.log('🔍 In-App Google Sign-In Debug Info:');
 ...
 */
export const smartGoogleSignIn = async (options?: InAppGoogleSignInOptions) => {
  console.log('🚀 Starting smart Google Sign-In...');
  
  if (isCapacitorEnvironment()) {
    try {
      console.log('📱 Using in-app browser for Google Sign-In');
      return await initiateInAppGoogleSignIn(options);
    } catch (error) {
