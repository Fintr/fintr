/**
 * Native Google Sign-In for iOS using window.open with specific parameters
 * This provides a better mobile experience than full redirect
 */

import { verifyState, generateRandomState } from './google-signin';

export interface NativeGoogleSignInOptions {
  redirectUri?: string;
  state?: string;
}

/**
 * Initiates Google Sign-In using native window.open for mobile
 * This creates a popup-like experience on mobile devices
 */
export const initiateNativeGoogleSignIn = async (options?: NativeGoogleSignInOptions) => {
  ...
  console.log('🔍 Native Google Sign-In Debug Info:');
  ...
  // Use window.open with mobile-optimized parameters
  ...
  // Return a promise that resolves when the popup closes or completes
 ...
 */
export const smartNativeGoogleSignIn = async (options?: NativeGoogleSignInOptions) => {
  console.log('🚀 Starting smart native Google Sign-In...');
  
  if (isMobileDevice()) {
    try {
      console.log('📱 Using native popup for Google Sign-In on mobile');
      return await initiateNativeGoogleSignIn(options);
    } catch (error) {
      console.warn('⚠️ Native popup failed, falling back to redirect:', error);
